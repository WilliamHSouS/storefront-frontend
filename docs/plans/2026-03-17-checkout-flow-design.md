...
4. **CSRF protection on checkout mutations.** Client-side write requests are unsigned — `BaseLayout.astro` strips `hmacSecret` from `window.__MERCHANT__`, and `sdk-stub.ts` skips HMAC signing when the secret is `undefined`. This is the existing behavior for all client-side cart mutations (add to cart, update quantity, etc.) which already work in production. The backend does not enforce HMAC on storefront API endpoints. CSRF protection relies on `SameSite=Lax` cookies (Vercel default) and the backend should validate the `Origin` header on state-mutating requests.
